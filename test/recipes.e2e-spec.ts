...
describe('recipes (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    const c = await ctx
      ...
      .send({ name: 'Троянда', kind: 'FLOWER', unit: 'PIECE', purchasePriceKopiyky: 1000, salePriceKopiyky: 2000 })
      .expect(201);
    catId = c.body.id;
  });
  afterAll(() => ctx.close());
  ...
  it('rejects saving an empty bouquet as a template (400)', async () => {
    const b = await ctx.authed('post', '/api/bouquets').send({ title: 'Порожній' }).expect(201);
    await ctx
  ...
  it('saves a bouquet composition as a template', async () => {
    const b = await ctx.authed('post', '/api/bouquets').send({ title: 'Класика' }).expect(201);
    await ctx.authed('post', `/api/bouquets/${b.body.id}/lines`).send({ catalogItemId: catId, quantity: 5 }).expect(201);
    const res = await ctx
      .authed('post', '/api/recipes/from-bouquet')
      .send({ bouquetId: b.body.id, name: 'Класичний букет', notes: '5 троянд' })
      .expect(201);
  ...
  it('lists templates', async () => {
    ...
    expect(res.body.find((r: { id: string }) => r.id === recipeId)).toBeDefined();
  });
  ...
  it('instantiates a template into a fresh DRAFT bouquet with CURRENT catalog prices', async () => {
    ...
    await ctx.authed('patch', `/api/catalog/${catId}`).send({ salePriceKopiyky: 3000 }).expect(200);

    const res = await ctx.authed('post', `/api/recipes/${recipeId}/use`).expect(201);
